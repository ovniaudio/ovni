...
// =============================================================================
// [latency][horizon] — gate H8 del sello: latencia REPORTADA == REAL (±1 sample).
// HORIZON tiene latencia OLA REAL = N samples (2048 @48k, el frame entero del STFT):
// el processor la declara con setLatencySamples() Y el dry pasa por el MISMO retardo
// dentro del motor. Método (multi-bloque porque 2048 > un bloque): MIX=0 → sólo el
// dry retrasado; impulso en el sample 0; el pico de |out| a lo largo de VARIOS
// bloques marca el retardo real. Imprime LATENCY_REPORTED= / LATENCY_REAL=.
// =============================================================================
TEST_CASE ("HORIZON: latencia reportada == real (dry retrasado N, MIX=0)", "[latency][horizon]")
{
    ...
    const int kBlocks = 8;   // 8×512 = 4096 ≥ 2·N: el impulso retrasado N cae adentro seguro.
