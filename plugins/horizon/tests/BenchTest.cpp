...
// =============================================================================
// [bench][horizon] — CPU% wall-clock del HorizonProcessor REAL: 10 000 bloques
// 48k/512 (house-standard: budget STFT ≤ 6% single instance). MECANISMO ENCENDIDO
// (regla de oro): FREEZE on + RATE en SYNC (el gate vivo recalcula la envolvente
// raised-cosine cada frame + el whisper randomiza fase: el costo REAL, no el caso
// quieto). Imprime CPU_PCT= (measure-check.sh).
// =============================================================================
TEST_CASE ("HORIZON bench: CPU% sobre 10000 bloques 48k/512 (freeze + gate)", "[bench][horizon]")
{
    ...
    set (pid::FREEZE,   1.0f);   // congelado (resíntesis del frame cada hop: el caso de uso real)
    set (pid::RATESYNC, 1.0f);   // gate vivo enganchado al tempo
    set (pid::WHISPER,  0.3f);   // whisper activo (randomización de fase por frame)
    set (pid::SPREAD,   0.6f);
